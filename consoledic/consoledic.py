...
def getword(word):
    ...
    url = "http://xtk.azurewebsites.net/BingDictService.aspx?{params}".format(params=params)
    f = request(url)
...
def prettyprint(wordjson):
    if wordjson and wordjson["word"]:
        if wordjson["pronunciation"] and wordjson["pronunciation"]["AmE"]:
            print("{word}---/{pronunciation}/".format(word=wordjson["word"],
                                                      pronunciation=wordjson["pronunciation"]["AmE"].encode("utf8")))
