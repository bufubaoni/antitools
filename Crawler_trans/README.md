...
## 介绍
经典计算机理论强调高效率的算法，尽可能快速的完成计算。但是大多数的网络相关程序的速度瓶颈并非在计算上，打开很多的慢连接，或者不频繁的事件。这个项目面对一个非常困难的挑战：有效的处理大数量的网络请求。当前处理方法是使用异步 i/o 或者叫做 "async"(异步)。

这一章节将会建立一个简单的web爬虫。因为要处理诸多的网络响应但是仅仅做少量计算，所以爬虫将以异步应用为原型。一次获取的页面越多，计算完成的就越快。如果每个请求都分配一个线程，则随着并发请求的增加，在他耗尽socket之前它将耗尽内存或者其他线程的资源，而使用异步i/o避免了线程的开销。

此项目分为三个阶段：

1. 做一个异步事件轮询，并写一个带有回调事件循环的爬虫，这样做的效率很高，但是带来的是复杂度的提升和难以维护。
2. 使用python的协程，可以同时达到效率和扩展性，我们实现了简单的协程协议，在python中使用生成器方法。
3. 我们使用"asyncio"标准库中协程的全部特性，然后使用异步队列来进行调度。

## 任务
一个web爬虫爬取网站的所有页面，或存档或索引。从根url开始，获取网站的所有链接，然后将这些添加到队列中。直到无连接可以爬去为止，此时队列也为空。

我们可以同时下载许多页面来加速这个过程。一旦爬虫爬取了新的连接，他会单独的使用socket获取新页面提取其中内容，响应完成后解析页面，并添加新的连接给队列。由于会出现无响应的情况，使得高并发的性能降低，因此我们应当降低并发的数量，并将其余链接保留在队列中，直到之前的请求完成。

## 传统方法
如何实现爬虫并发。一般来说，我们将建立一个线程池。每一个线程都会通过一个socket下载一个页面，例如从`xkcd.com`下载一个页面。
...
def fetch(url):
    ...
    q.add(links)
```
默认socket的操作为空，当线程调用方法 `connect` 或者 `recv`的时候会阻塞在此。为了一次下载多个页面，我们需要很多线程。

一个复杂的应用通过将空闲的线程保留在线程池中，然后再对其复用，来降低线程创建的消耗，socket也有同样的连接池。

但是，线程很昂贵，并且操作系统对线程，用户，和机器有着各种各样的硬件上的限制。在Jesse系统，一个py的线程将占用50k的内存，不可能启用1w以上的线程。如果我们并发操作socket，我们会在耗尽soket之前耗尽线程资源。每个线程的开销或者系统对线程的限制都将是瓶颈。
...
Kegel在1999年创建了"C10K"组织。万计的连接听上去不多，貌似只是量变。正如之前所说，对于C10K线程是不切实际的，即使现代机器性能提高。尽管，我们的爬虫可以很好的使用线程。但是面对巨大规模的应用，例如十万级别的连接，面对的问题仍然是，它的规模超过了大多数的系统可以创建的socket数量，如果不使用多线程，将如何实现呢？

## 异步
异步i/o框架在一个线程上使用非阻塞socket并行执行。使用异步爬虫的时候，我们需要将socket设置其为非阻塞：
